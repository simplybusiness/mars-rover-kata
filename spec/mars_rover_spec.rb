...
RSpec.describe 'mars rover' do
  ...
  invalid_directions.each do |invalid_direction|
    it "cannot have an invalid direction e.g. #{invalid_direction.inspect}" do
      ...
      end.to raise_error(ArgumentError, 'Starting direction is invalid and must be N, E, S, or W')
    end
  end
  ...
  invalid_starting_positions.each do |invalid_starting_position|
    it "cannot have invalid starting position e.g. #{invalid_starting_position}" do
      ...
      end.to raise_error(ArgumentError, 'Starting position is invalid and must be valid coordinates')
    end
  end
  ...
  it 'can move forward and backward in one command' do
    mars_rover = MarsRover.new(starting_position: Coordinates.new(0, 0), starting_direction: 'N')
    mars_rover.execute(%w[f b])
    expect(mars_rover).to be_located_at(Coordinates.new(0, 0))
  end

  it 'moves forwards and is facing east' do
    mars_rover = MarsRover.new(starting_position: Coordinates.new(0, 0), starting_direction: 'E')
    mars_rover.execute(['f'])
    expect(mars_rover).to be_located_at(Coordinates.new(1, 0))
  end

  it 'moves backwards and is facing east' do
    mars_rover = MarsRover.new(starting_position: Coordinates.new(0, 0), starting_direction: 'E')
    mars_rover.execute(['b'])
    expect(mars_rover).to be_located_at(Coordinates.new(-1, 0))
  end

  it 'moves forwards and is facing south' do
    mars_rover = MarsRover.new(starting_position: Coordinates.new(0, 0), starting_direction: 'S')
    mars_rover.execute(['f'])
    expect(mars_rover).to be_located_at(Coordinates.new(0, -1))
  end

  it 'moves backwards and is facing south' do
    mars_rover = MarsRover.new(starting_position: Coordinates.new(0, 0), starting_direction: 'S')
    mars_rover.execute(['b'])
    expect(mars_rover).to be_located_at(Coordinates.new(0, 1))
  end

  it 'moves forwards and is facing west' do
    mars_rover = MarsRover.new(starting_position: Coordinates.new(0, 0), starting_direction: 'W')
    mars_rover.execute(['f'])
    expect(mars_rover).to be_located_at(Coordinates.new(-1, 0))
  end

  it 'moves backwards and is facing west' do
    mars_rover = MarsRover.new(starting_position: Coordinates.new(0, 0), starting_direction: 'W')
    mars_rover.execute(['b'])
    expect(mars_rover).to be_located_at(Coordinates.new(1, 0))
  end
  ...
  it 'turns left multiple times' do
    mars_rover = MarsRover.new(starting_position: Coordinates.new(0, 0), starting_direction: 'N')
