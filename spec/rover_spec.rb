  ...
  describe '#move' do
    ...
    directions_mapping_right.each do |start_direction,expected_direction|
      it "does face #{expected_direction} when receives a single right command in the route and starting direction of #{start_direction}" do
        rover = Rover.new(direction: start_direction, coordinates:Coordinate.new(x=3, y=4, x_pos: 3, y_pos: 4))
        rover.move(['r'])
    ...
    it "does face North when receives a two right and two left commands in the route and starting direction of North" do
      rover = Rover.new(direction: 'N', coordinates:Coordinate.new(x=3, y=4, x_pos: 3, y_pos: 4))
      rover.move(['r','r','l','l'])
      expect(rover.direction).to eq('N')
    end

    directions_mapping_forward = { 'N' => Coordinate.new(x=3, y=5, x_pos: 3, y_pos: 5),
      'S' => Coordinate.new(x=3, y=3, x_pos: 3, y_pos: 3),
      'W' => Coordinate.new(x=2, y=4, x_pos: 2, y_pos: 4),
      'E' => Coordinate.new(x=4, y=4, x_pos: 4, y_pos: 4)
    }
    ...
    directions_mapping_backward = { 'N' => Coordinate.new(x=3, y=3, x_pos: 3, y_pos: 3),
      'S' => Coordinate.new(x=3, y=5, x_pos: 3, y_pos: 5),
      'W' => Coordinate.new(x=4, y=4, x_pos: 4, y_pos: 4),
      'E' => Coordinate.new(x=2, y=4, x_pos: 2, y_pos: 4)
    }
    ...
    xit "returns to the start position when instructed to travel the entire equator" do
      rover = Rover.new(direction: 'E',coordinates: Coordinate.new(x=0, y=0, x_pos: 0, y_pos: 0))
