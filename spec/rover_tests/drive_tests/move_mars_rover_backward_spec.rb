...
RSpec.describe Rover do
  it 'moves backward correctly when facing North' do
    ...
    expect { rov.move(commands: ['b']) }.to change(rov, :current_position)
      .from(Coordinates.new(x: 0, y: 0))
      .to(Coordinates.new(x: 0, y: -1))
  end
  ...
  it 'moves forward 10 times correctly' do
    ...
    expect { rov.move(commands: %w[b b b b b b b b b b]) }.to change(rov, :current_position)
      .from(Coordinates.new(x: 0, y: 0))
      .to(Coordinates.new(x: 0, y: -10))
  end
end
